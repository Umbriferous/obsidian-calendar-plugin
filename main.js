 ...
 */
async function tryToCreateMonthlyNote(date) {
    const { workspace } = window.app;
    const createFile = async () => {
...
async function openOrCreateMonthlyNote(date) {
    const { workspace } = window.app;
    const existingFile = getMonthlyNote_1(date, get_store_value(monthlyNotes));
    if (!existingFile) {
        tryToCreateMonthlyNote(date);
        return;
    }
    const leaf = workspace.getUnpinnedLeaf();
			...
			if (!mounted) {
				dispose = [
					listen(span0, "click", function () {
                        openOrCreateMonthlyNote(ctx[0]);
					}),
                    listen(span1, "click", function () {
                        openOrCreateYearlyNote(ctx[0]);
					}),
